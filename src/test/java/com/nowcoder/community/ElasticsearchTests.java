...
public class ElasticsearchTests {
    ...
    public void testDelete() {
        // discussRepository.deleteById(231);   //单条删除
    ...
    public void testSearchByRepository() {
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                .withQuery(QueryBuilders.multiMatchQuery("互联网寒冬", "title", "content"))   //搜索的关键字和字段
                .withSort(SortBuilders.fieldSort("type").order(SortOrder.DESC))    //排序规则
                .withSort(SortBuilders.fieldSort("score").order(SortOrder.DESC))
                .withSort(SortBuilders.fieldSort("createTime").order(SortOrder.DESC))
                .withPageable(PageRequest.of(0, 10))    //分页  查第0页，10个数据
                .withHighlightFields(        //高亮显示
                        new HighlightBuilder.Field("title").preTags("<em>").postTags("</em>"),        //高亮显示的字段
                        new HighlightBuilder.Field("content").preTags("<em>").postTags("</em>")       //高亮显示的字段
                ).build();

        // elasticTemplate.queryForPage(searchQuery, class, SearchResultMapper)
        // 底层获取得到了高亮显示的值, 但是没有返回.

        Page<DiscussPost> page = discussRepository.search(searchQuery);    //执行查询
        System.out.println(page.getTotalElements());    //一共有多少个数据匹配
        ...
        for (DiscussPost post : page) {              //显示数据
